...
function ResourceItem({ resource, maxWidth }: { resource: Resource; maxWidth?: string }) {
  if (resource.movie) {
    return (
...
function MainMedia({ media }: { media: Resource }) {
  if (media.movie) {
    return (
...
export default function ProjectViewer({ data, onBack }: Props) {
        ...
        {project.map((proj, i) => (
          ...
          >
            <div className="pv__project-header">
              ...
              <p className="pv__project-tools">{proj.tools}</p>
            </div>
